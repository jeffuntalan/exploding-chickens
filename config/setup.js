...
exports.check_values = function (config_storage) {
    spinner.start(`${chalk.bold.cyan('Setup')}: Checking configuration values`);
    ...
    if (!config_storage.has('webserver_port') || config_storage.get('webserver_port') === '') {
        ...
        spinner.warn(`${chalk.bold.cyan('Setup')}: "webserver_port" value in config.json set to default: "3000"`);
    }
    ...
    if (!config_storage.has('verbose_debug_mode') || config_storage.get('verbose_debug_mode') === '') {
        ...
        spinner.warn(`${chalk.bold.cyan('Setup')}: "verbose_debug_mode" value in config.json set to default: "false"`);
    }
    //Exit if the config values are not set properly
    if (invalid_config) {
        spinner.info(`${chalk.bold.cyan('Setup')}: Please check "env.json" and configure the appropriate values`);
        process.exit(0);
    } else {
        spinner.succeed(`${chalk.bold.cyan('Setup')}: Configuration values have been propagated`);
    }
}